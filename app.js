(() => {
  ...
  let saved = null;
  try { saved = JSON.parse(localStorage.getItem(KEY) || localStorage.getItem('we-v5') || localStorage.getItem('we-v4') || 'null'); } catch (_) {}

  const S = {
    lat: clamp(finite(saved?.lat, 17.385), -90, 90),
    lng: clamp(finite(saved?.lng, 78.4867), -180, 180),
    zoom: clamp(finite(saved?.zoom, 16), 3, 19),
    weather: null,
    items: Array.isArray(saved?.items) ? saved.items : [],
    visited: new Set(Array.isArray(saved?.visited) ? saved.visited : []),
    markers: [],
    heading: 0,
    manualMode: ['auto','day','night'].includes(saved?.manualMode) ? saved.manualMode : 'auto',
    weatherBusy: false,
  ...
  const map = L.map('map', { zoomControl: false, preferCanvas: true, fadeAnimation: true, worldCopyJump: true }).setView([S.lat, S.lng], S.zoom);
  ...
  const carto = L.tileLayer('https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png', { maxZoom: 20, subdomains: 'abcd', attribution: '© OpenStreetMap contributors © CARTO' });
