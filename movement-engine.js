(() => {
  ...
  // World Explorer v25 — live movement / trip trail engine.
  // Loaded before app.js so it can capture Leaflet's map and Explorer Core marker.
  if (!window.L) return;
  ...
  function loadTrip() {
    try {
      ...
      points = saved.points.slice(-MAX_POINTS);
      totalMeters = Number(saved.totalMeters) || 0;
      last = points.length ? points[points.length - 1] : null;
    } catch (_) {}
  }

  function saveTrip() {
    try { localStorage.setItem(KEY, JSON.stringify({ points: points.slice(-MAX_POINTS), totalMeters })); } catch (_) {}
  }

  function injectStyle() {
    if (document.getElementById('movement-engine-style')) return;
    const style = document.createElement('style');
    style.id = 'movement-engine-style';
    style.textContent = `
  ...
  function createStatus() {
    ...
    statusEl.className = 'we-track-status';
  ...
  function captureMap() {
    ...
    L.map = function(...args) {
      ...
      map = instance;
      window.__WORLD_EXPLORER_MAP__ = instance;
      setTimeout(init, 0);
      return instance;
    };
  }
  ...
  function captureCore() {
    ...
    L.marker = function(...args) {
      ...
      const opts = args[1] || {};
      if (Number(opts.zIndexOffset) === 3000 && !core) {
        core = marker;
        window.__WORLD_EXPLORER_CORE__ = marker;
        setTimeout(init, 0);
      }
      return marker;
    };
  ...
  function init() {
    ...
    createStatus();
    if (!route) {
      route = L.polyline(points.map(p => [p.lat, p.lng]), { className:'we-route-glow', color:'#55dfff', weight:7, opacity:.9, lineCap:'round', lineJoin:'round', interactive:false, pane:'overlayPane' }).addTo(map);
      routeInner = L.polyline(points.map(p => [p.lat, p.lng]), { color:'#9df1ff', weight:2.5, opacity:.95, lineCap:'round', lineJoin:'round', interactive:false, pane:'overlayPane' }).addTo(map);
    }
    ...
    if (points.length) {
      route.setLatLngs(points.map(p => [p.lat,p.lng]));
      routeInner?.setLatLngs(points.map(p => [p.lat,p.lng]));
    }
  }
  ...
  function setCore(lat, lng, animate = true) {
    ...
    const from = core.getLatLng(), to = L.latLng(lat,lng);
    const d = haversine({lat:from.lat,lng:from.lng},{lat,lng});
    if (d < .5) return;
    const duration = clamp(d * 20, 280, 1100), start = performance.now();
    cancelAnimationFrame(animationFrame);
    const tick = now => {
      const t = clamp((now-start)/duration,0,1), eased=t*(2-t);
      core.setLatLng([from.lat+(to.lat-from.lat)*eased, from.lng+(to.lng-from.lng)*eased]);
  ...
  function orient(next) {
    if (!core || !last) return;
    const deg = bearing(last,next), root = core.getElement()?.querySelector('.core-wrap');
    if (root) root.style.setProperty('--heading',deg+'deg');
  }
  ...
  function acceptPosition(pos) {
    ...
    if (!last) {
      last=next; points.push(next); route?.setLatLngs(points.map(p=>[p.lat,p.lng])); routeInner?.setLatLngs(points.map(p=>[p.lat,p.lng])); setCore(lat,lng,false); status(); saveTrip(); return;
    }
    ...
    route?.setLatLngs(points.map(p=>[p.lat,p.lng])); routeInner?.setLatLngs(points.map(p=>[p.lat,p.lng]));
  ...
  function toastSafe(message){
    ...
    el.textContent=message; el.classList.add('show'); clearTimeout(toastSafe.t); toastSafe.t=setTimeout(()=>el.classList.remove('show'),2600);
  }
  ...
  function hookLocation(){
    ...
    const wrapped=function(success,error,options){
      const ok=typeof success==='function'?success:()=>{};
      return original(pos=>{ok(pos);setTimeout(startTracking,0);},error,options);
    };
