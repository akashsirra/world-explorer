(() => {
  ...
  const FOLLOW_AFTER_METERS=12;

  let map=null,core=null,watchId=null,tracking=false,follow=true;
  let route=null,routeGlow=null,last=null,points=[],totalMeters=0,statusEl=null,raf=0;
  ...
  function styles(){
    if(document.getElementById('we-travel-style'))return;
    const s=document.createElement('style');s.id='we-travel-style';s.textContent=`
      ...
      .we-travel-status.on{opacity:1;transform:none}.we-travel-dot{width:8px;height:8px;border-radius:50%;background:#58e1ff;box-shadow:0 0 14px #58e1ff;animation:weTravelPulse 1.2s infinite}.we-travel-status .meters{opacity:.7;text-transform:none;letter-spacing:.02em;font-weight:700}
      @keyframes weTravelPulse{50%{transform:scale(.55);opacity:.5}}
      @media(max-width:520px){.we-travel-status{top:82px;left:18px}}
      @media(prefers-reduced-motion:reduce){.we-travel-dot{animation:none}}
  ...
  function capture(){
    ...
    const oldMarker=L.marker;L.marker=function(...args){const m=oldMarker.apply(this,args),o=args[1]||{};if(Number(o.zIndexOffset)===3000&&!core){core=m;window.__WORLD_EXPLORER_CORE__=m;setTimeout(init,0);}return m;};
  }

  function init(){
    if(!map||!document.body)return;styles();ui();
    if(!route){
      routeGlow=L.polyline(coords(),{className:'we-travel-glow',color:'#24cfff',weight:11,opacity:.28,lineCap:'round',lineJoin:'round',interactive:false,pane:'overlayPane'}).addTo(map);
      route=L.polyline(coords(),{color:'#72eaff',weight:5,opacity:.98,lineCap:'round',lineJoin:'round',interactive:false,pane:'overlayPane'}).addTo(map);
    }
    route.setLatLngs(coords());routeGlow.setLatLngs(coords());
    if(!map.__weTravelEvents){
      map.__weTravelEvents=true;
  ...
  function moveCore(lat,lng,animate=true){
    if(!core)return;
  ...
  function position(pos){
    ...
    if(d>NEW_TRIP_METERS){points=[];totalMeters=0;last=null;route?.setLatLngs([]);routeGlow?.setLatLngs([]);save();return position(pos);}
    if(d>MAX_JUMP_METERS&&(!Number.isFinite(c.speed)||c.speed<0||c.speed>25))return;
    heading(next);moveCore(lat,lng,true);points.push(next);if(points.length>MAX_POINTS)points=points.slice(-MAX_POINTS);totalMeters+=d;last=next;
    route?.setLatLngs(coords());routeGlow?.setLatLngs(coords());
  ...
  async function autoStartIfAllowed(){
    ...
    try{if(navigator.permissions?.query){const p=await navigator.permissions.query({name:'geolocation'});if(p.state==='granted')startTracking();p.onchange=()=>{if(p.state==='granted')startTracking();};}}
  ...
  capture();load();
  const boot=()=>{init();hookButtons();autoStartIfAllowed();};
